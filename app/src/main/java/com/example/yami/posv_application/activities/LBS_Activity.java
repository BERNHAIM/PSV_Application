...
/**
 * LBS부분이 내위치 찍히고 그주변에 위험지역 확인가능한 맵
 **/
...
public class LBS_Activity extends BaseActivity
    ...
    private static final String NOTIFICATION_CHANNEL_ID = "psv_notification_channel";
    ...
    public String loadJSONFromAsset() {
        String json = null;
        try {
            InputStream is = getAssets().open("dangerArea.json");
...
//            LoginActivity.this.startActivity(intent);//Activity로 넘어감
    ...
    public void onLocationChanged(Location location) {
        ...
        //lat/lng: (37.3059057,127.0043627)
        Log.d(TAG, "onLocationChanged : "+currentPosition);
        ...
        //위험지역 안에 있으면 push알림
        pushNearDangerArea(currentPosition);
    }
    ...
    private void showDialogForPermission(String msg) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(LBS_Activity.this);
        builder.setTitle("알림");
    ...
    /* 위험 지역 반경에 들어가면 푸시알람 */
    public void pushNearDangerArea(LatLng latLng){
        double distance;

        JSONObject position = null;
        JSONArray positions = null;
        String locations = loadJSONFromAsset();

        // 위험지역 좌표를 저장하는 arraylist
        ArrayList<double[]> dangerAreaList = new ArrayList<double[]>();

        Location start = new Location("Start");
        Location destination = new Location("Destination");

        try {
            position = new JSONObject(locations);
            positions = position.getJSONArray("location");

            for (int i = 0; i < positions.length(); i++) {
                double x = positions.getJSONObject(i).getDouble("x");
                double y = positions.getJSONObject(i).getDouble("y");
                // x  : Latitude , y : Longitude
                double[] desinations = {x,y};

                dangerAreaList.add(desinations);

                Log.d("destination"+i,Arrays.toString(dangerAreaList.get(i)));

                /*
                 * 이제 각 위험지역과 내 현재 위치를 비교해서 1000m(반경)안인 경우
                 * push알람을 보내주어 경고를 보내주게끔 만들어보자.
                 */
                //lat/lng: (37.3059057,127.0043627)
                start.setLatitude(latLng.latitude);
                start.setLongitude(latLng.longitude);

                destination.setLatitude(x);
                destination.setLongitude(y);
    ...
    /*Notification Manager(Push알림 설정)*/
    public void NotificationManaging(){
        Resources res = getResources();

        int notifi_id = 1;
        Intent notificationIntent = new Intent(this, LBS_Activity.class);
        notificationIntent.putExtra("notificationId",notifi_id);
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            NotificationChannel notificationChannel = new NotificationChannel(NOTIFICATION_CHANNEL_ID,"PSV Notification",
        ...
        String bigContent = "당신은 현재 폭력이 일어난 위험지역에 있습니다.\n확인하시려면 터치해주세요.";

        builder.setContentTitle("PSV가 알려드립니다.")
                .setContentText("당신은 현재 폭력이 일어난 위험지역에 있습니다.\n확인하시려면 터치해주세요.")
                .setTicker("PSV NOTIFICATION")
                .setSmallIcon(getNotificationIcon())
                .setLargeIcon(BitmapFactory.decodeResource(res,getNotificationIcon()))
        ...
        style.bigText(bigContent).setBigContentTitle("PSV가 알려드립니다.");
    ...
    private int getNotificationIcon(){
        ...
        return useWhiteIcon ? R.drawable.ic_alarm : R.drawable.ic_alarm ;
    }
